...
export const PlayerManagerV2 = ({ onClose }: { onClose: () => void }) => {
  const { players, addPlayer, removePlayer } = useGameStoreV2()
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-md bg-slate-900 rounded-t-3xl sm:rounded-2xl border-t border-x sm:border border-indigo-500/30 p-6 pb-8 shadow-2xl relative flex flex-col max-h-[90vh]"
      >
        ...
        <div className="shrink-0 mb-6 relative">
          <h3 className="text-xl font-black text-white text-center flex items-center justify-center gap-2 uppercase tracking-wide">
            <Icons.Users className="w-6 h-6 text-indigo-400" /> Quản Lý Hội Nhậu
          </h3>
          <p className="text-sm text-slate-400 text-center mt-1">
            Tổng cộng: <span className="text-indigo-300 font-bold">{players.length}</span> người
          </p>
        </div>

        <form onSubmit={handleAdd} className="flex gap-2 mb-6 shrink-0 relative z-10 w-full">
            ...
            className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:grayscale text-white px-6 rounded-xl font-bold transition-all shadow-md active:scale-95 flex items-center justify-center min-w-[80px]"
            >
            Thêm
            </button>
        </form>
