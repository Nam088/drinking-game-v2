...
import { Player } from "@/store/useGameStoreV2"

interface PlayerListV2Props {
  players: Player[]
        ...
        <button
          ...
          className={`px-3 py-3 rounded-xl text-sm font-bold transition-all border flex items-center gap-2 ${
            selectedPlayerId === p.id 
              ? "bg-indigo-600/20 text-indigo-300 border-indigo-500/50 shadow-inner ring-1 ring-indigo-500" 
              : "bg-slate-800/50 text-slate-400 border-white/5 hover:bg-slate-800 hover:text-slate-200"
          }`}
        >
          <div className={`w-6 h-6 shrink-0 rounded-full flex items-center justify-center transition-colors ${selectedPlayerId === p.id ? 'bg-indigo-500/30' : 'bg-slate-700/50'}`}>
            <Icons.User className={`w-3.5 h-3.5 ${selectedPlayerId === p.id ? 'text-indigo-400' : 'text-slate-500'}`} />
          </div>
