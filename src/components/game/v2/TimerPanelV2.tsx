...
const TimerItemV2 = ({ timer }: { timer: Timer }) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-slate-800/80 rounded-2xl p-5 border relative overflow-hidden transition-colors ${
        isFinished ? "border-rose-500/80 bg-rose-950/40 shadow-[0_0_20px_rgba(244,63,94,0.4)] animate-pulse" :
        timer.isRunning ? "border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.2)]" : "border-white/10"
      }`}
    >
      ...
      <div className="relative z-10 flex flex-col gap-3">
        ...
        <div className="flex justify-between items-center">
          {isEditingLabel ? (
            <input
              ...
              className="bg-slate-900/50 text-white px-3 py-1 rounded border border-indigo-500/50 outline-none w-[60%] text-sm font-bold"
            />
          ) : (
            <h3 
              onClick={() => setIsEditingLabel(true)}
              className="text-white font-bold text-sm tracking-wide truncate max-w-[70%] cursor-pointer hover:text-indigo-300 transition-colors flex items-center gap-2"
            >
              {timer.label}
          ...
          <button
            ...
            className="text-slate-500 hover:text-rose-400 p-1.5 rounded-full hover:bg-slate-700/50 transition-colors"
          >
          ...
          {isEditingTime ? (
            <div className="flex flex-col items-center">
              <input
                ...
                className="bg-slate-900 text-center text-4xl sm:text-5xl font-display tracking-wider text-white border-b-2 border-indigo-500 outline-none w-[160px]"
                placeholder="00:00"
              />
        ...
        <div className="w-full h-1.5 bg-slate-900/50 rounded-full overflow-hidden mb-2">
          <motion.div 
            className={`h-full ${isFinished ? 'bg-rose-500' : 'bg-indigo-500'}`}
            initial={{ width: '100%' }}
        ...
        <div className="flex justify-center gap-4">
          <button
            ...
            className={`w-14 h-14 rounded-full flex items-center justify-center transition-all shadow-md ${
              isFinished 
                ? "bg-slate-800 text-white hover:bg-slate-700 ring-2 ring-rose-500/50 shadow-rose-900/40"
              : timer.isRunning 
...
export const TimerPanelV2 = ({ isOpen, onClose }: TimerPanelV2Props) => {
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="p-4 border-b border-indigo-500/20 flex justify-between items-center bg-slate-800/50 shrink-0">
              <h2 className="text-xl font-black text-white uppercase tracking-widest flex items-center gap-2">
                <Icons.Timer className="w-5 h-5 text-indigo-400" />
                Đồng Hồ (V2)
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 custom-scrollbar">
              <AnimatePresence mode="popLayout">
                {timers.map((timer) => (
                  <TimerItemV2 key={timer.id} timer={timer} />
                ))}
              </AnimatePresence>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                <motion.button
                  ...
                  className="py-3 rounded-xl border border-slate-700 hover:border-indigo-500/50 hover:bg-slate-800/80 text-slate-300 transition-colors flex flex-col items-center justify-center gap-1 font-bold text-xs"
                >
                  <Icons.Timer className="w-4 h-4 text-indigo-400" /> 30s
                </motion.button>
                <motion.button
                  layout
                  ...
                  className="py-3 rounded-xl border border-slate-700 hover:border-indigo-500/50 hover:bg-slate-800/80 text-slate-300 transition-colors flex flex-col items-center justify-center gap-1 font-bold text-xs"
                >
                  <Icons.Timer className="w-4 h-4 text-indigo-400" /> 1 Phút
                </motion.button>
                <motion.button
                  layout
                  ...
                  className="py-3 rounded-xl border border-slate-700 hover:border-indigo-500/50 hover:bg-slate-800/80 text-slate-300 transition-colors flex flex-col items-center justify-center gap-1 font-bold text-xs"
                >
                  <Icons.Timer className="w-4 h-4 text-indigo-400" /> 3 Phút
                </motion.button>
                <motion.button
                  layout
                  ...
                  className="py-3 rounded-xl border border-slate-700 hover:border-indigo-500/50 hover:bg-slate-800/80 text-slate-300 transition-colors flex flex-col items-center justify-center gap-1 font-bold text-xs"
                >
                  <Icons.Timer className="w-4 h-4 text-indigo-400" /> 5 Phút
                </motion.button>
              </div>
              ...
              <motion.button
                ...
                className="w-full py-4 rounded-2xl border-2 border-dashed border-slate-700 hover:border-indigo-500/50 hover:bg-slate-800/50 text-slate-400 hover:text-indigo-300 transition-colors flex items-center justify-center gap-2 font-bold mb-4"
              >
